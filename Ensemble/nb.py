...
def fun():
    ...
    classifier_digits = GaussianNB()
    classifier_wine = GaussianNB()
    classifier_olivetti_faces = GaussianNB()
    ...
    for i in y_test_digits:
        if y_test_digits[i] == targets_predicted_digits[i]:
            j += 1
    digits_acc = "Accuracy of GaussianNB Digits:        " + str(100 * (j/len(y_test_digits)))
    j = 0
    for i in y_test_wine:
        if y_test_wine[i] == targets_predicted_wine[i]:
            j += 1
    wine_acc = "Accuracy of GaussianNB Wine:          " + str(100 * (j/len(y_test_wine)))
    j = 0
    for i in y_test_olivetti_faces:
        if y_test_olivetti_faces[i] == targets_predicted_olivetti_faces[i]:
            j += 1
    olivetti_faces_acc = "Accuracy of GaussianNB Olivetti Faces: " + str(100 * (j/len(y_test_olivetti_faces)))
