...
def fun():
    ...
    classifier_digits = MLPClassifier(solver='adam', alpha=1e-5, hidden_layer_sizes=(100,), random_state=0)
    classifier_wine = MLPClassifier(solver='adam', alpha=1e-5, hidden_layer_sizes=(100,), random_state=0)
    classifier_olivetti_faces = MLPClassifier(solver='adam', alpha=1e-5, hidden_layer_sizes=(100,), random_state=0)
    ...
    for i in y_test_digits:
        if y_test_digits[i] == targets_predicted_digits[i]:
            j += 1
    digits_acc = "Accuracy of Neural Net Digits:        " + str(100 * (j/len(y_test_digits)))
    j = 0
    for i in y_test_wine:
        if y_test_wine[i] == targets_predicted_wine[i]:
            j += 1
    wine_acc = "Accuracy of Neural Net Wine:          " + str(100 * (j/len(y_test_wine)))
    j = 0
    for i in y_test_olivetti_faces:
        if y_test_olivetti_faces[i] == targets_predicted_olivetti_faces[i]:
            j += 1
    olivetti_faces_acc = "Accuracy of Neural Net Olivetti Faces: " + str(100 * (j/len(y_test_olivetti_faces)))
